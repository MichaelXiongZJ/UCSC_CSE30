...
Original file is located at
    https://colab.research.google.com/drive/1P5X2G5Z-yZoz87velZfN-f3DHfj8_phm

# Overall Score

You received 50 out of 50 points.
...
def kink(p0, p1):
    ...
    q0 = p0
    q1 = (p0[0] + 1/4*delta[0], p0[1] + 1/4*delta[1])
    q2 = (p0[0] + 1/2*delta[0] - 1/4*delta[1], p0[1] + 1/2*delta[1] + 1/4*delta[0])
    q3 = (p0[0] + 3/4*delta[0], p0[1] + 3/4*delta[1])
#    print(q0, q1, q2, q3)
    return (q0, q1, q2, q3)
...
### For this question, you received 0 out of 0 points.
...
def kink_polygon(poly):
    ...
    for a in range(len(poly)-1):
      pts += kink(poly[a], poly[a+1])
    pts += kink(poly[-1], poly[0])
    return pts
...
def poly_area(polygon):
    ...
    for a in range(len(polygon)-1):
      area += (polygon[a+1][0] - polygon[a][0])*(polygon[a][1] + polygon[a+1][1])/2
    return area
...
def poly_perimeter(polygon):
    ...
    # YOUR CODE HERE
    peri = math.sqrt((polygon[0][0] - polygon[-1][0])**2 + (polygon[0][1] - polygon[-1][1])**2)
    for a in range(len(polygon)-1):
      peri += math.sqrt((polygon[a+1][0] - polygon[a][0])**2 + (polygon[a+1][1] - polygon[a][1])**2)
    return peri
...
# YOUR CODE HERE
poly1 = [(0, 0), (1, 2), (2, 0)]
abs(poly_perimeter(poly1) - 6.472) < 0.01
