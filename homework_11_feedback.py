# -*- coding: utf-8 -*-
"""Homework_11_feedback

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1CFrSUllSql7E1p3qirFh79rYjifPWME8
...
def scheduler_available_tasks(self):
    ...
    # YOUR CODE HERE
    set_tasks = set()
    for a in self.uncompleted:
      done = True
      for b in self.predecessors[a]:
        if b not in self.completed_tasks:
          done = False
      if done:
        set_tasks.add(a)
    return set_tasks
...
s.add_task('b', ['c', 'd'])
s.add_task('c', ['d', 'e'])
assert_equal(s.available_tasks, {'d', 'e'})
r = s.mark_completed('e')
assert_equal(r, set())
...
def dependency_scheduler_redo(self, t):
    ...
    while len(unchecked) > 0:
      a = unchecked.pop()
      self.completed_tasks.discard(a)
      set_tasks.add(a)
      unchecked.update(self.successors[a] - set_tasks)
    return set_tasks
...
def dependency_scheduler_cooking_redo(self, v):
    ...
    while len(unchecked) > 0:
      ...
      self.completed_tasks.discard(a)
      set_tasks.add(a)
      unchecked.update(self.predecessors[a] - set_tasks)
    set_tasks.update(dependency_scheduler_redo(self,v))
    return set_tasks
...
s.mark_completed('f')
s.cooking_redo('d')
assert_equal(s.completed_tasks, {'c', 'f'})
### END HIDDEN TESTS
...
class AND_OR_Scheduler(object):
    ...
    def __init__(self):
        ...
        self.or_tasks = set()
        self.successors = defaultdict(set)
        self.or_successors = defaultdict(set)
        self.predecessors = defaultdict(set)
        self.or_predecessors = defaultdict(set)
        self.completed_tasks = set()

        self.has_or_pred_tasks = set()
    ...
    def add_or_task(self, t, dependencies):
        ...
        # YOUR CODE HERE
        self.or_tasks.add(t)
        self.has_or_pred_tasks.add(t)
        self.or_tasks.update(dependencies)
        self.or_predecessors[t] = set(dependencies)
        for u in dependencies:
            self.or_successors[u].add(t)

    @property
    def done(self):
        # YOUR CODE HERE
        return self.completed_tasks == self.tasks|self.or_tasks
    ...
    def available_tasks(self):
        ...
        # YOUR CODE HERE
        set_tasks = set()
        all_tasks = self.tasks | self.or_tasks
        uncompleted = all_tasks.difference(self.completed_tasks)
        for a in uncompleted:
          if a not in self.has_or_pred_tasks:
            done = True
            for b in self.predecessors[a]:
              if b not in self.completed_tasks:
                done = False
            ...
            if self.or_predecessors[a] == set():
              done = True
            else:
              for b in self.or_predecessors[a]:
    ...
    def mark_completed(self, t):
        ...
        if t in self.tasks:
          ...
          for a in self.or_successors[t]:
            done = False
            for b in self.or_predecessors[a]:
              if b in self.completed_tasks and a not in self.completed_tasks:
                done = True
            if done:
              set_tasks.add(a)
        return set_tasks
...
s = AND_OR_Scheduler()
s.add_and_task('a', ['b', 'c', 'd'])
assert_equal(s.available_tasks, {'b', 'c', 'd'})
r = s.mark_completed('b')
assert_equal(r, set())
assert_equal(s.available_tasks, {'c', 'd'})
r = s.mark_completed('c')
assert_equal(r, set())
assert_equal(s.available_tasks, {'d'})
r = s.mark_completed('d')
...
s = AND_OR_Scheduler()
s.add_or_task('a', ['b', 'c', 'd'])
assert_equal(s.available_tasks, {'b', 'c', 'd'})
r = s.mark_completed('b')
assert_equal(r, {'a'})
assert_equal(s.available_tasks, {'a', 'c', 'd'})
### END HIDDEN TESTS
...
assert 'a' not in s.available_tasks
r = s.mark_completed('c1')
assert 'c' not in s.available_tasks
r = s.mark_completed('c2')
assert 'c' in s.available_tasks
s.mark_completed('c')
assert 'a' in s.available_tasks
s.mark_completed('a')
assert s.available_tasks == {'b2'}
### END HIDDEN TESTS