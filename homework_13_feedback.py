# -*- coding: utf-8 -*-
"""Homework_13_feedback

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1l-nmMvxT7Idz_0hzcYoasyvpb7G_FpO3
...
def sudoku_propagate_cell(self, ij):
    ...
    x0 = i//3*3
    y0 = j//3*3
    for ii in range(x0,x0+3):
      for jj in range(y0,y0+3):
        if ii != i and jj != j:
...
### BEGIN HIDDEN TESTS
tsd = Sudoku.from_string('[[[5], [3], [2], [6], [7], [8], [9], [1, 2, 4], [2, 3]], [[6], [7], [1, 2, 4, 7], [1, 2, 3], [9], [5], [3], [1, 2, 4], [8]], [[1, 2], [9], [8], [3], [4], [1, 2], [5], [6], [7]], [[8], [5], [9], [1, 9, 7], [6], [1, 4, 9, 7], [4], [2], [3]], [[4], [2], [6], [8], [5], [3], [7], [9], [1]], [[7], [1], [3], [9], [2], [4], [8], [5], [6]], [[1, 9], [6], [1, 5, 9, 7], [9, 5, 7], [3], [9, 7], [2], [8], [4]], [[9, 2], [8], [9, 2, 7], [4], [1], [9, 2, 7], [6], [3], [5]], [[3], [4], [2, 3, 4, 5], [2, 5, 6], [8], [6], [1], [7], [9]]]')
assert_equal(tsd.propagate_cell((3, 7)), set())
assert_equal(tsd.m[0][7], {1, 4})
# tsd.show(details=True)
assert_equal(tsd.propagate_cell((3, 8)), {(0, 8)})
### END HIDDEN TESTS
...
sd.show(details=True)
sd.to_string()
sdd = Sudoku.from_string('[[[5], [3], [4], [6], [7], [8], [1, 9], [1, 9], [2]], [[6], [7], [2], [1], [9], [5], [3], [4], [8]], [[1], [9], [8], [3], [4], [2], [5], [6], [7]], [[8], [5], [9], [7], [6], [1], [4], [2], [3]], [[4], [2], [6], [8, 9], [5], [3], [7, 8, 9], [7, 9], [1]], [[7], [1], [3], [8, 9], [2], [4], [8, 9], [5, 9], [6]], [[9], [6], [1], [5], [3], [7], [2], [8], [4]], [[2], [8], [7], [4], [1], [9], [6], [3], [5]], [[3], [4], [5], [2], [8], [6], [1, 7], [1, 7], [9]]]')
assert_equal(sd, sdd)
### END HIDDEN TESTS
...
### BEGIN HIDDEN TESTS
assert_equal(occurs_once_in_sets([{1}]), {1})
assert_equal(occurs_once_in_sets([]), set())
assert_equal(occurs_once_in_sets([{1, 2, 3}, {1, 2}]), {3})
assert_equal(occurs_once_in_sets([{1, 2, 3}, {1, 2}, set()]), {3})
assert_equal(occurs_once_in_sets([{1, 2, 3}, {1, 2}, {1}, {1, 2}, {}]), {3})
### END HIDDEN TESTS
...
def sudoku_where_can_it_go(self):
    ...
    # YOUR CODE HERE
    for i in range(9):
      list_of_row = [self.m[i][j] for j in range(9)]
      x = occurs_once_in_sets(list_of_row)
      for a in x:
        for coord in range(9):
          if a in self.m[i][coord] and len(self.m[i][coord]) != 1:
            self.m[i][coord] = {a}
            newly_singleton.add((i, coord))
            break
    for j in range(9):
      list_of_row = [self.m[i][j] for i in range(9)]
      x = occurs_once_in_sets(list_of_row)
      for a in x:
        for coord in range(9):
          if a in self.m[coord][j] and len(self.m[coord][j]) != 1:
            self.m[coord][j] = {a}
            newly_singleton.add((coord, j))
            break
    for ii in range(0, 9, 3):
      for jj in range(0, 9, 3):
        list_of_row = [self.m[i][j] for i in range(ii, ii+3) for j in range(jj, jj+3)]
        x = occurs_once_in_sets(list_of_row)
        for a in x:
          for ic in range(ii, ii+3):
            for jc in range(jj, jj+3):
              if a in self.m[ic][jc] and len(self.m[ic][jc]) != 1:
                self.m[ic][jc] = {a}
...
### For this question, you received 20 out of 20 points.
...
# sd.show(details=True)
sd.to_string()
sdd = Sudoku.from_string('[[[4], [3], [5], [2], [6], [9], [7], [8], [1]], [[6], [8], [2], [5], [7], [1], [4], [9], [3]], [[1], [9], [7], [8], [3], [4], [5], [6], [2]], [[8], [2], [6], [1], [9], [5], [3], [4], [7]], [[3], [7], [4], [6], [8], [2], [9], [1], [5]], [[9], [5], [1], [7], [4], [3], [6], [2], [8]], [[5], [1], [9], [3], [2], [6], [8], [7], [4]], [[2], [4], [8], [9], [5], [7], [1], [3], [6]], [[7], [6], [3], [4], [1], [8], [2], [5], [9]]]')
assert_equal(sd, sdd)
### END HIDDEN TESTS
