...
Original file is located at
    https://colab.research.google.com/drive/1NFSDuIF4CSxlKpE36DOjmEJZAwnGj0sz
...
* Second, download the notebook in .ipynb format (File > Download .ipynb) and upload the .ipynb file to [this form](https://docs.google.com/forms/d/e/1FAIpQLSfw1cbq6idG_jSN7X7Bx1xWUMP78cpCbjwnyVfHmT0hFhg0rw/viewform?usp=sf_link). 
...
def expr_compute_gradient(self, de_loss_over_de_e=1):
    ...
    de_e_over_de_child = self.operator_gradient()
    for i in range(len(self.children)):
        if isinstance(self.children[i], Expr):
            self.children[i].compute_gradient(de_loss_over_de_e * de_e_over_de_child[i])
Expr.compute_gradient = expr_compute_gradient
...
e.zero_gradient()
e.compute_gradient()
assert abs(vx.gradient - 0.3333) < 0.01
assert abs(vy.gradient - -1.3333) < 0.01
### END HIDDEN TESTS
