...
Original file is located at
    https://colab.research.google.com/drive/1tSFwBE3_im4lfWS5NNKvtuFeGYj9nyQk

# Overall Score

You received 0 out of 50 points.
...
class Bus(object):
    ...
    def __init__(self, queue_ring, max_capacity, geton_time, nextstop_time,
        ...
        self.numstops = len(self.queue_ring)
    ...
    def onboard(self):
        ...
        # YOUR CODE HERE
        return self.ppl_on_bus

    @property
    def occupancy(self):
        """Returns the number of passengers on the bus."""
        # YOUR CODE HERE
        return len(self.ppl_on_bus)
    ...
    def arrive(self, stop_idx, time=None):
        ...
        stop_idx += 1
        return [Event(self.get_off(), delay = self.geton_time)]
    ...
    def get_off(self, time = None):
      new_event = []
      if self.space_on_bus > 0:
          for i in self.queue_ring[self.current_stop].people:
              x = self.queue_ring[self.current_stop].people.pop(i)
              x.start_bus
              self.people_on.append(x)
              new_event.append(Event(self.get_on(), delay = self.geton_time))
      (self.current_stop + 1) % len(self.queue_ring)
      new_event.append(Event(self.arrive(), delay = self.nextstop_time, args = [self.current_stop]))
      return new_event

    def get_on(self, time = None):
      new = []
      for i in range(self.occupancy):
          if self.people_on[i].destination == self.current_stop:
              self.people_on[i].arrived
              x = self.people_on.pop(i)
              new.append(Event(self.get_off(), delay = self.geton_time))
      new.append(Event(self.get_on(), delay = self.geton_time))
      return new
    
    # def move_on(self, time = None):

    #   return [Event(self.move_on, time = None)]
...
### For this question, you received 0 out of 20 points.
...
assert 12 < np.average(travel_times) < 30
assert 25 < np.quantile(travel_times, 0.9) < 50
assert 30 < np.average(total_times) < 60
assert 40 < np.quantile(total_times, 0.9) < 80
assert 15 < np.average(wait_times) < 30
assert 25 < np.quantile(wait_times, 0.9) < 60
...
### For this question, you received 0 out of 30 points.
...
### BEGIN HIDDEN TESTS
# Let us check that the times are reasonable.
travel_times1 = [p.travel_time for p in sim1.have_arrived]
total_times1 = [p.elapsed_time for p in sim1.have_arrived]
wait_times1 = [p.wait_time for p in sim1.have_arrived]
print("Travel:", np.average(travel_times1), np.quantile(travel_times1, 0.90))
print("Total:", np.average(total_times1), np.quantile(total_times1, 0.90))
print("Wait:", np.average(wait_times1), np.quantile(wait_times1, 0.90))
print("---")
travel_times2 = [p.travel_time for p in sim2.have_arrived]
total_times2 = [p.elapsed_time for p in sim2.have_arrived]
wait_times2 = [p.wait_time for p in sim2.have_arrived]
print("Travel:", np.average(travel_times2), np.quantile(travel_times2, 0.90))
print("Total:", np.average(total_times2), np.quantile(total_times2, 0.90))
print("Wait:", np.average(wait_times2), np.quantile(wait_times2, 0.90))
distances = [bus_distance(pi, pj, num_stops=sim2.num_stops)
             for pi, pj in zip(sim2.positions[0], sim2.positions[1])]
print("Distances", np.average(distances[5000:]))
assert 10 < np.average(travel_times2) < 20
assert 20 < np.quantile(travel_times2, 0.9) < 30
assert 25 < np.average(total_times2) < 35
assert 38 < np.quantile(total_times2, 0.9) < 55
assert 10 < np.average(wait_times2) < 18
assert 20 < np.quantile(wait_times2, 0.9) < 30
assert np.average(distances[5000:]) < 1
### END HIDDEN TESTS